...
public class MainActivity extends AppCompatActivity {
    ...
    public void launchDetailsFragmentForTwoPane(Movie movie) {
        if (mIsTwoPane) {
            ScrollView scrollView = (ScrollView) findViewById(R.id.details_container);
            ...
            if (scrollView.getChildCount() <= 0) { //if there is no current detail fragment being displayed
                getFragmentManager().beginTransaction().add(R.id.details_container, detailsFragment).commit();
            } else { //replace current fragment
